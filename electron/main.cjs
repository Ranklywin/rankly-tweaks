...
app.whenReady().then(async () => {
  ...
  handle('source', async id => { const item=[...tweaks,...guides].find(g=>g.id===id); if(!item?.source || !item.source.startsWith('https://')) throw new Error('No source for this guide.'); await shell.openExternal(item.source); });
  handle('export', async () => {
